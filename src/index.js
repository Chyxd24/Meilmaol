// ============================================================
// MediaFairy Tmail - PROTECTED VERSION v3.0
// With Telegram Bot Authentication & Approval System
// ============================================================

// ======================== CONFIG ========================
// Environment Variables Required:
// BOT_TOKEN, ADMIN_USER_ID, CHANNEL_ID, GROUP_ID, 
// ADMIN_USERNAME, ADMIN_PASSWORD, EMAILS (KV binding)
// ========================================================
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === "POST" && url.pathname === "/email") {
      // This is handled by the email() function below
...
async function handleMessage(msg, env) {
  ...
  // Start command
  if (text === "/start" || text === "/start@" + (await getBotInfo(env)).username) {
...
async function handleStart(userId, username, firstName, chatId, env) {
  // Check if user already exists and approved
  ...
  // New user - check membership first
...
async function handleReject(userId, chatId, msgId, env) {
  ...
  // Save as blocked or just rejected
...
async function handleBlock(adminId, targetId, env) {
  ...
  await tgApi(env, "sendMessage", {
    chat_id: targetId,
    text: `🚫 *Akses Diblokir*\n\nAkun kamu telah diblokir oleh admin. Hubungi admin untuk informasi lebih lanjut.`,
    parse_mode: "Markdown",
  });
}
...
async function handleWebmail(request, env) {
  ...
  if (params.get("action") === "login") {
    const loginType = params.get("type"); // 'user' or 'admin'
    ...
    if (loginType === "admin") {
      ...
      if (username === env.ADMIN_USERNAME && password === env.ADMIN_PASSWORD) {
        ...
        return new Response("", {
          ...
          headers: {
            Location: "/",
            "Set-Cookie": `tmail_auth=${token}; ${cookieFlags}`,
            ...noCacheHeaders,
          },
        });
      } else {
  ...
  if (params.get("action") === "logout") {
    return new Response("", {
      ...
      headers: {
        Location: "/?page=login",
        "Set-Cookie": `tmail_auth=; Path=/; Max-Age=0`,
        ...noCacheHeaders,
      },
    });
  }
  ...
  if (!authCookie) {
    return new Response("", {
      status: 302,
      headers: { Location: "/?page=login", ...noCacheHeaders },
    });
  }
...
async function handleUserWebmail(request, env, authData, getCookie, cookieFlags, noCacheHeaders) {
  ...
  const userData = await getUserData(env, userId);
  const currentEmail = userData.email || `${userId}@tmail.user`;

  // ... (sama dengan existing webmail logic tapi dengan proteksi)
  // Saya akan gunakan logic existing yang sudah ada di _worker.js
  // tapi dengan modifikasi untuk user auth
  ...
  const cleanStyles = `
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .email-text { font-size: 16px; font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; color: #111; }
    ...
    .auth-badge { display: inline-flex; align-items: center; gap: 6px; background: #e8f5e9; color: #2e7d32; padding: 6px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; margin-bottom: 16px; }
    .auth-badge svg { width: 14px; height: 14px; }
    .logout-btn { position: absolute; top: 20px; right: 20px; padding: 8px 14px; background: #ff3b30; color: #fff; border: none; border-radius: 8px; font-size: 12px; font-weight: 600; cursor: pointer; text-decoration: none; }
  `;
  ...
  try {
        ...
        <body>
          <div class="container" style="position:relative;">
            ...
            <div class="view-card">
              ...
              <div style="padding-top:16px; border-top:1px solid #e0e4e8;">
                ${contentHTML}
              </div>
    ...
    let emails = [];
    for (const key of allKeys) {
      if (key.name.startsWith("att_") || key.name.startsWith("user:") || key.name.startsWith("key:") || key.name.startsWith("pending:") || key.name === "stats" || key.name === "saved_domains" || key.name === "account_history" || key.name === "bot_info" || key.name === "channel_invite" || key.name === "group_invite") continue;
      try {
        const data = await env.EMAILS.get(key.name);
        if (data) {
...
async function handleAdminWebmail(request, env, authData, getCookie, cookieFlags, noCacheHeaders) {
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
      ...
      <body>
        <div class="container">
          ...
          <table>
            <thead><tr><th>User ID</th><th>Name</th><th>Username</th><th>Status</th><th>Key</th><th>Approved At</th></tr></thead>
            <tbody>${rows}</tbody>
          </table>
        </div>
      </body></html>`;
  ...
  if (params.get("admin") === "approve") {
    ...
    await handleApprove(targetId, env.ADMIN_USER_ID, null, env);
    return new Response("", { status: 302, headers: { Location: "/?admin=pending", ...noCacheHeaders } });
  }

  if (params.get("admin") === "reject") {
    const targetId = params.get("id");
    await handleReject(targetId, env.ADMIN_USER_ID, null, env);
    return new Response("", { status: 302, headers: { Location: "/?admin=pending", ...noCacheHeaders } });
  }
